...
COMMENT ON COLUMN sys_file.path IS '存储路径（相对存储根目录）';
COMMENT ON COLUMN sys_file.size IS '文件大小（字节）';
COMMENT ON COLUMN sys_file.content_type IS '文件 MIME 类型';
COMMENT ON COLUMN sys_file.storage_type IS '存储类型：local-本地磁盘（OSS 等留扩展）';
COMMENT ON COLUMN sys_file.create_at IS '创建时间（上传时间）';
COMMENT ON COLUMN sys_file.update_at IS '更新时间';
COMMENT ON COLUMN sys_file.create_by IS '创建人ID（上传人）';
