...
## 更新摘要
**变更内容**
- 更新了Docker Compose编排配置章节，增加了服务依赖管理和错误处理机制
- 添加了更清晰的启动指令和依赖验证流程
- 完善了本地开发环境的快速启动指南
- 增强了故障排查指南中的Docker Compose相关问题
...
**更新** 改进了Docker Compose集成，提供更清晰的启动指令和更好的依赖错误处理
...
- 健康检查
  - PostgreSQL: 使用 `pg_isready` 命令检查数据库可用性
  - Redis: 使用 `redis-cli ping` 命令检查Redis响应
- 使用方式
  - docker compose up -d
...
DC->>PG : 启动并挂载数据卷
DC->>RD : 启动并挂载数据卷
Note over DC,PG : 等待PostgreSQL健康检查通过
Note over DC,RD : 等待Redis健康检查通过
...
### 本地开发快速启动
**更新** 提供了更清晰的启动指令和依赖验证流程

- 前置检查
  - 确保已安装 Docker 和 Docker Compose
  - 验证端口 5432 和 6379 未被占用
- 启动依赖
  - docker compose up -d
...
- 启动应用
  ...
  - 或使用脚本：./mvnw spring-boot:run
- 验证
  - 访问 Swagger UI：http://localhost:8080/swagger-ui.html
  - 使用内置管理员账号登录：admin@example.com / admin123456
- 停止服务
...
## 故障排查指南
**更新** 增强了Docker Compose相关的故障排查内容

- Docker Compose 问题
  - 端口冲突：检查 5432 和 6379 端口是否被占用
  - 健康检查失败：查看 `docker compose logs postgres` 和 `docker compose logs redis`
  - 依赖启动顺序：确保PostgreSQL和Redis健康检查通过后应用才启动
  - 数据卷权限：检查宿主机目录权限和数据卷挂载
- 启动失败
  - 检查数据库/Redis 连通性与凭据
  - 查看 Flyway 迁移是否成功
  - 确认应用能连接到健康检查通过的依赖服务
...
- [docker-compose.yaml:15-19](file://docker-compose.yaml#L15-L19)
