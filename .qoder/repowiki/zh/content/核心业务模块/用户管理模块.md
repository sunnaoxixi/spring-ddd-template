...
## 更新摘要
**变更内容**   
- 增强了权限控制机制，使用system:user:read和system:user:write权限进行访问控制
- 实现了用户逻辑删除功能，替代物理删除
- 更新了API接口的权限注解配置
- 完善了删除用户的业务流程，包含会话强制下线处理
...
**更新** 本模块现已支持细粒度的权限控制和逻辑删除功能，提供更安全的用户管理体验。

## 项目结构
用户管理模块遵循 DDD 分层：
- 适配层（Adaptor）：HTTP 控制器接收请求并委派应用服务，实现细粒度权限控制
...
graph TB
subgraph "适配层"
C["UserController<br/>@SaCheckPermission"]
end
subgraph "应用层"
...
subgraph "基础设施层"
R["UserRepositoryImpl<br/>逻辑删除"]
L["LockFactory/LevelLock"]
DB["MySQL(MyBatis-Flex)<br/>@Column(isLogicDelete=true)"]
end
C --> A1
C --> A2
A1 --> D1
...
- [UserPO.java:55-58](file://src/main/java/com/sunnao/spring/ddd/template/infrastructure/system/user/mysql/po/UserPO.java#L55-L58)
...
- 仓储实现 UserRepositoryImpl：PO 与聚合根的转换、分页查询、事务性组合写入/删除（含角色关联），支持逻辑删除。
...
**更新** 仓储实现现已支持逻辑删除功能，通过MyBatis-Flex的@Column(isLogicDelete=true)注解实现自动的逻辑删除处理。
...
participant Controller as "UserController<br/>@SaCheckPermission"
...
Controller->>Controller : 验证 system : user : write 权限
alt 权限不足
...
sequenceDiagram
participant Client as "客户端"
participant Controller as "UserController<br/>@SaCheckPermission(system : user : read)"
...
Controller->>Controller : 验证 system : user : read 权限
alt 权限不足
Controller-->>Client : 403 Forbidden
else 权限通过
...
- [UserRepositoryImpl.java:73-87](file://src/main/java/com/sunnao/spring/ddd/template/infrastructure/system/user/repository/UserRepositoryImpl.java#L73-87)
...
### 权限控制机制（新增）
**新增** 用户管理模块现已实现细粒度的权限控制，使用 Sa-Token 框架进行访问控制：

- **读取权限**：`system:user:read` - 用于查询用户详情和分页查询用户列表
- **写入权限**：`system:user:write` - 用于创建、更新、状态变更和删除用户
...
flowchart TD
A["HTTP请求到达"] --> B{"权限检查"}
B --> |system:user:read| C["查询接口<br/>getUserDetail/queryUserPage"]
B --> |system:user:write| D["写操作接口<br/>create/update/changeStatus/delete"]
B --> |无权限| E["返回403 Forbidden"]
C --> F["执行查询逻辑"]
D --> G["执行写操作逻辑"]
F --> H["返回查询结果"]
G --> I["返回操作结果"]
...
图表来源
- [UserController.java:18-21](file://src/main/java/com/sunnao/spring/ddd/template/adaptor/system/user/input/UserController.java#L18-L21)
- [UserController.java:36-37](file://src/main/java/com/sunnao/spring/ddd/template/adaptor/system/user/input/UserController.java#L36-L37)
- [UserController.java:85-86](file://src/main/java/com/sunnao/spring/ddd/template/adaptor/system/user/input/UserController.java#L85-L86)
...
### 逻辑删除机制（新增）
**新增** 用户管理模块现已支持逻辑删除功能，替代传统的物理删除：

- **实现方式**：通过 MyBatis-Flex 的 `@Column(isLogicDelete = true)` 注解实现
- **删除字段**：`deleted` 字段，0表示正常，1表示已删除
- **自动处理**：查询时自动过滤已删除记录，删除时自动设置 deleted=1
- **事务保证**：逻辑删除与角色关联清理在同一事务中执行
...
sequenceDiagram
participant Client as "客户端"
participant Controller as "UserController<br/>@SaCheckPermission(system : user : write)"
participant App as "UserAppServiceImpl"
participant Domain as "UserDomainServiceImpl"
participant Repo as "UserRepositoryImpl"
participant DB as "MySQL"
Client->>Controller : DELETE /api/system/users/{id}
Controller->>Controller : 验证 system : user : write 权限
alt 权限不足
Controller-->>Client : 403 Forbidden
else 权限通过
Controller->>App : deleteUser(requestDTO)
App->>App : 参数自校验
App->>Domain : deleteUser(deleteParam)
Domain->>Repo : deleteWithRoles(userId, operatorId)
Repo->>DB : UPDATE sys_user SET update_by=?, deleted=1 WHERE id=?
Repo->>DB : 清理用户角色关联
DB-->>Repo : 删除成功
Repo-->>Domain : 删除完成
Domain-->>App : 删除成功
App->>App : 强制下线用户会话
App-->>Controller : 返回删除结果
...
- [UserAppServiceImpl.java:122-149](file://src/main/java/com/sunnao/spring/ddd/template/application/system/user/scenario/UserAppServiceImpl.java#L122-L149)
- [UserDomainServiceImpl.java:155-182](file://src/main/java/com/sunnao/spring/ddd/template/domain/system/user/service/UserDomainServiceImpl.java#L155-L182)
...
- 外部依赖：
  ...
  - 数据库：MyBatis-Flex 分页与条件构造，支持逻辑删除。
  - 权限：基于 Sa-Token 的细粒度权限控制。

```mermaid
graph LR
UC["UserController<br/>@SaCheckPermission"] --> UA["UserAppServiceImpl"]
UC --> UQ["UserQueryAppServiceImpl"]
UA --> UD["UserDomainServiceImpl"]
UD --> UR["UserRepositoryImpl<br/>逻辑删除"]
UD --> RR["RoleRepository"]
UD --> EV["UserCreatedEvent"]
EV --> UL["UserCreatedListener"]
UR --> DB["MySQL<br/>@Column(isLogicDelete=true)"]
```
...
- **新增** 逻辑删除优化：通过 MyBatis-Flex 自动处理 deleted 字段，避免手动更新带来的性能开销。
...
- 常见错误码与定位：
  ...
  - 数据库异常：查看仓储层的异常包装与日志。
  - **新增** 权限不足：检查用户是否拥有相应的 system:user:read 或 system:user:write 权限。
  - **新增** 逻辑删除异常：检查 deleted 字段配置和 MyBatis-Flex 逻辑删除配置。
- 建议排查步骤：
  - 核对请求参数是否符合 DTO 校验规则。
  - 观察领域服务日志输出，定位异常分支。
  - 检查仓储层 SQL 与分页参数。
...
## 结论
用户管理模块以聚合根为核心，严格封装状态机与业务规则，配合应用层场景编排与仓储事务性组合操作，实现了高内聚、低耦合的可维护架构。通过锁、唯一性校验、批量填充与异步事件等手段，兼顾了正确性与性能。**新增的细粒度权限控制和逻辑删除功能进一步提升了系统的安全性和数据管理能力**。API 层职责清晰，便于扩展与维护。
...
#### 权限控制说明
**新增** 所有接口均受细粒度权限控制：
- `GET /api/system/users/{id}` - 需要 `system:user:read` 权限
- `GET /api/system/users/page` - 需要 `system:user:read` 权限  
- `POST /api/system/users` - 需要 `system:user:write` 权限
- `PUT /api/system/users/{id}` - 需要 `system:user:write` 权限
- `PUT /api/system/users/{id}/status` - 需要 `system:user:write` 权限
- `DELETE /api/system/users/{id}` - 需要 `system:user:write` 权限
...
- 行为：权限验证 → 参数校验 → 领域服务创建（加锁、唯一性、默认角色、加密、持久化、事件发布）→ 返回成功
...
- 行为：权限验证 → 参数校验 → 领域服务变更状态 → 若禁用则强制下线该用户全部会话 → 返回成功
...
- 行为：权限验证 → 参数校验 → 领域服务逻辑删除并清理角色关联 → 强制下线该用户全部会话 → 返回成功
- **特性**：采用逻辑删除，不会真正从数据库中移除记录，而是设置 deleted=1
...
- 行为：权限验证 → 参数校验 → 查询聚合根 → 批量填充角色标识 → 返回成功
...
- 行为：权限验证 → 参数校验 → 分页查询 → 批量填充角色标识 → 返回成功

**更新** 所有接口现在都集成了细粒度的权限控制，确保只有具备相应权限的用户才能执行对应的操作。逻辑删除功能的引入提供了更好的数据管理和审计能力。
