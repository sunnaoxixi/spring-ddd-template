...
纯计算模式的业务逻辑完全由 DomainService 承载，通常不需要 Output Adaptor。但当 Application 层需要**跨领域查询外部数据**作为计算输入时，需通过 Output Adaptor 获取。Adaptor 参数与返回值遵循 1.4 节核心规则。
...
规则+计算模式的规则数据通过 Repository 从数据库或配置中心加载，通常不需要 Output Adaptor。但当 Application 层需要**跨领域查询外部数据**作为计算输入时，需通过 Output Adaptor 获取。Adaptor 参数与返回值遵循 1.4 节核心规则。
